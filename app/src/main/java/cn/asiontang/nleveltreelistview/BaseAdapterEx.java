 ...
 */
public abstract class BaseAdapterEx<T> extends Filter implements ListAdapter
        , Filterable
        , SpinnerAdapter
        , Filter.FilterListener
{
    ...
    /**
     * <p><b>所以，只要子类需要实现{@link #filter(CharSequence, FilterListener)}，一般都需要重载此函数 }</b></p>
     * 此函数的作用为：如AutoCompleteTextView，当输入文本，出现检索列表时，点击其中的某一项，则界面需要将该项的“啥玩意”显示到编辑框的？
     *
     * @return 系统默认是选择的那一项.toString()
     */
     ...
     * <br/>
     * 真正的过滤参数需要通过 BaseAdapterEx.getFilterConstraintArgs()获得！
     * <br/>
     * <p>调用filter过滤时可以通过实现FilterListener接口达到监听过滤完成后的回调 </p>
     * <ol>
     * <li>使用{@link #filter(Object...)}时可以直接在adapter子类中重载 onFilterComplete(int
     * count)回调方法 </li>
     * <br/>
     * <li>或者使用{@link #setFilterListener(FilterListener)}设置一个监听后再调用{@link #filter(Object...)}过滤
     * </li>
     * </ol>
     */
    public void filter(final Object... constraintArgs)
    {
        ...
        super.filter("真正的过滤参数需要通过 BaseAdapterEx.getFilterConstraintArgs()获得！", this.mFilterListener == null ? this : this.mFilterListener);
    }
    ...
    protected FilterResults performFiltering(final CharSequence constraint)
    {
        ...
        {
            //LogEx.e(this.getClass().getSimpleName(), "performFiltering", e);
        }
        return result;
    }
     ...
     * 设置过滤完成后回调监听
     */
    public void setFilterListener(final FilterListener listener)
    {
